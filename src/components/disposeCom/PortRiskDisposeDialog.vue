      ...
      <template v-else>
        ...
        >
          <div class="dispose-port-card-head">
            <div class="dispose-port-card-head-main">
              <div class="dispose-port-title-row">
                <h4 class="dispose-port-title">
                  {{ portDialogTitleText }} {{ portItem.port }}
                </h4>
                <el-tag
              ...
              >
                <span
                  v-if="getPortCardAssetIpText()"
                  class="dispose-risk-title-sub-item"
                >
                ...
                >
                <span
                  v-if="getPortCardServiceText(portItem)"
                  class="dispose-risk-title-sub-item"
                >
            ...
            <el-button
              link
              class="dispose-port-toggle"
              @click="toggleExpand(portItem.id)"
            >
              {{ expandedIds.includes(portItem.id) ? '收起' : '详情' }}
              <i
                :class="
                  expandedIds.includes(portItem.id)
                    ? 'el-icon-arrow-up'
                    : 'el-icon-arrow-right'
                "
              ></i>
          ...
          <el-collapse-transition>
            <div
              v-show="expandedIds.includes(portItem.id)"
              class="dispose-port-expand"
            >
              <div
                v-if="getPortCardProtocolText(portItem)"
              ...
              >
                <div class="dispose-port-vuln-api-head">
                  ...
                  <el-tag size="small" type="warning" effect="plain">
                    共 {{ normalizePortItemVulnIds(portItem).length }} 个
                  </el-tag>
                </div>
                <div
                  ...
                  >
                    <div class="dispose-port-vuln-api-card-body">
                      ...
                      <el-tag
                        ...
                        effect="plain"
                      >
                        {{ formatVerifyVulnLevel(vulnApiRow.vuln_level) }}
                      </el-tag>
                    </div>
                  </el-card>
                  <div
              ...
              <div class="dispose-port-actions">
                ...
                <el-button
                  size="small"
                  type="primary"
                  @click="emitDownloadReport(portItem)"
                >
                  生成报告
                </el-button>
              </div>
          ...
          >
            ...
            >
              <template #default="{ row }">
                <el-tag
                  ...
                  effect="plain"
                >
                  {{ formatVerifyVulnLevel(row.level) }}
                </el-tag>
              </template>
            </el-table-column>
            <el-table-column
            ...
            >
              <template #default="{ row }">
                ...
                >
                  {{ formatVerifyVulnStatus(row.vulnStatus) }}
                </span>
              </template>
            </el-table-column>
            <el-table-column
...
export default {
  ...
  methods: {
    ...
    getPortCardServiceText (portItem) {
      const item = portItem || {}
      const serviceRaw = item.service != null ? String(item.service).trim() : ''
    ...
    resolveVerifyVulnLevel (plugin, item) {
      if (plugin && plugin.levelStr !== undefined && plugin.levelStr !== null && plugin.levelStr !== '') {
        return plugin.levelStr
      }
      if (item && item.vuln_level !== undefined && item.vuln_level !== null && item.vuln_level !== '') {
        return item.vuln_level
      }
      if (item && item.level !== undefined && item.level !== null && item.level !== '') {
        return item.level
      }
      return ''
    },
    resolveVerifyVulnStatus (item) {
      if (item && item.vuln_status !== undefined && item.vuln_status !== null && item.vuln_status !== '') {
        return item.vuln_status
      }
      if (item && item.vulnStatus !== undefined && item.vulnStatus !== null && item.vulnStatus !== '') {
        return item.vulnStatus
      }
      if (item && item.status !== undefined && item.status !== null && item.status !== '') {
        return item.status
      }
      return ''
    },
      ...
      return list.map((item, idx) => {
        ...
        return {
          ...
          name:
            ...
            item.title ||
            '-',
          level: this.formatVerifyVulnLevel(this.resolveVerifyVulnLevel(plugin, item)),
      ...
      this.sendPortRetestWsRequest(requestPayload, responseData => {
        ...
        .then(responseData => {
          if (
            ...
            responseData &&
            (Number(responseData.code) === 0 || Number(responseData.process) >= 100)
          ) {
            this.verifyProgress = 100
          }
          if (responseData && responseData.code != null && Number(responseData.code) === 0) {
